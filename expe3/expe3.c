...
/* Strip whitespace from the start and end of STRING.  Return a pointer
 into STRING. */
char* stripwhite(char* string)  // 去掉两端空白字符
{
    register char *s, *t;
...
void getRedirect() {
    ...
    for (int pIndex = 0; pIndex < progCntr; pIndex++) {
        if (pIndex != progCntr - 1 && progCntr > 1) // 不是最后一个子程序，且有多个程序，开放写管道
            prog[pIndex].tubeOut = 1;  // 如果使用了管道，那么必然要开放管道
        if (pIndex != 0 && progCntr > 1)// 不是第一个子程序，且有多个程序，开放读管道
        ...
        for (int i = 0; i < prog[pIndex].tmpCntr; i++) {
            if (strcmp("<", prog[pIndex].tmp[i]) == 0) {
                ...
                strcpy(prog[pIndex].tmp2[prog[pIndex].argCntr],
                       prog[pIndex].tmp[i]);
                // 更新该子程序的args总数
                ++prog[pIndex].argCntr;
            }
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        signal(
            SIGPIPE,
            (sighandler_t)
                catch_signal);  // 设置捕获错误信号
                                //  perror("SIGINT");
                                //  //如果系统调用signal成功执行，输出”SIGINT”
        if (pid < 0)  // 建立子进程失败?
        {
...
// 父进程代码
            ...
            prog = (struct Program*)malloc(
                M_LENGTH *
                sizeof(
                    struct
                    Program));  // 当前指令处理完成，清理本条指令对应的子程序数组
...
void processerFunc() {
    ...
    struct Program* progCreate =
        (struct Program*)malloc(M_LENGTH * sizeof(struct Program));
...
// 不用等待子进程结束。否则，就会导致“双重阻塞”，没有人能来唤醒他！
    ...
    exit(
        0);  // 必须手动exit，否则无法自动结束该进程的执行，导致用户主进程一直等待该进程的结束。
}
// 每个子程序用子进程执行
pid_t exceProcess(int pIndex) {
    pid_t pid;
...
// 报告父子进程进程号
        ...
        if (pIndex == progCntr - 1 && prog[pIndex].fileStdOut == 2)
            redirect_file(1, prog[pIndex].stdOutFile,
        ...
        if (pIndex == progCntr - 1 && prog[pIndex].fileErrOut == 2)
            redirect_file(1, prog[pIndex].errOutFile,
                          O_RDWR | O_APPEND | O_CREAT);
